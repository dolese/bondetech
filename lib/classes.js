...
const STREAM_STATUSES = ["active", "inactive"];
const DEFAULT_STREAM_CAPACITY = 50;
...
const validateStream = (value) => {
  const stream = normalizeStream(value);
  if (!/^[A-Z]$/.test(stream)) {
    throw new Error("Stream must be one letter from A to Z");
  }
  return stream;
};

const normalizeStreamCapacity = (value, fallback = DEFAULT_STREAM_CAPACITY) => {
  const parsed = Number.parseInt(value ?? fallback, 10);
  if (!Number.isFinite(parsed) || parsed < 1 || parsed > 500) {
    throw new Error("Stream capacity must be between 1 and 500 students");
  }
  return parsed;
};

const normalizeStreamStatus = (value, fallback = "active") => {
  const status = String(value || fallback).trim().toLowerCase();
  if (!STREAM_STATUSES.includes(status)) {
    throw new Error("Stream status must be active or inactive");
  }
  return status;
};

const normalizeClassTeacher = (value) => String(value || "").trim().slice(0, 120);
...
const normalizePhoneCollection = (primaryValue, listValue) => {
...
const normalizeCompositeConfig = (input = {}) => {
  ...
  return Object.fromEntries(
    Object.entries(input).flatMap(([exam, config]) => {
      ...
      const excludedSubjects = Array.from(
        new Set(
          (Array.isArray(config?.excludedSubjects) ? config.excludedSubjects : [])
            .map(normalizeSubjectName)
            .filter(Boolean)
        )
      );
      return [[examKey, { partnerExam, excludedSubjects }]];
    }),
  );
};
...
const parseClass = (doc) => {
  ...
  return {
    ...
    streamCapacity: Number(data.stream_capacity || 0),
    streamStatus: STREAM_STATUSES.includes(String(data.stream_status || "active").toLowerCase())
      ? String(data.stream_status || "active").toLowerCase()
      : "active",
    classTeacher: normalizeClassTeacher(data.class_teacher),
...
const parseStudent = (doc, classId, examType = DEFAULT_EXAM_TYPE) => {
  ...
  const conduct =
    data.conduct && typeof data.conduct === "object"
      ? data.conduct
      : {};
  const optionalSubjects = Array.isArray(data.optional_subjects)
    ? data.optional_subjects.map((entry) => String(entry || "").trim()).filter(Boolean)
    : Array.isArray(data.optionalSubjects)
    ? data.optionalSubjects.map((entry) => String(entry || "").trim()).filter(Boolean)
    : [];
  ...
  return {
    ...
    parentPhone: normalizeTzPhone(data.parent_phone || data.parentPhone || data.guardianPhone || ""),
    address: data.address || "",
    previousSchool: data.previous_school || "",
    // Per-exam scores only. The legacy top-level `scores` mirrors the default
    // exam, so it is the fallback for the default exam alone; any other exam
    // with no saved marks returns blank instead of leaking another exam's data.
    scores:
      Array.isArray(examScores[examType])
        ? examScores[examType]
...
const ensureUniqueClass = async (db, { year, form, stream = DEFAULT_STREAM, excludeId = "" }) => {
  ...
  const existing = snapshot.docs.find(
    (doc) =>
      doc.id !== excludeId &&
      !doc.data().archived &&
      normalizeStream(doc.data().stream, "") === normalizeStream(stream, "")
  );
  if (existing) {
...
const createClassRecord = async (db, payload = {}) => {
  ...
  const stream = validateStream(payload.stream || schoolInfo.stream || DEFAULT_STREAM);
  const streamCapacity = normalizeStreamCapacity(payload.streamCapacity ?? payload.capacity);
  const streamStatus = normalizeStreamStatus(payload.streamStatus ?? payload.status, "active");
  const classTeacher = normalizeClassTeacher(payload.classTeacher);
...
const updateClassRecord = async (db, classId, payload = {}) => {
  ...
  if (payload.stream !== undefined) {
    updates.stream = validateStream(payload.stream);
  }
  if (payload.streamCapacity !== undefined || payload.capacity !== undefined) {
    updates.stream_capacity = normalizeStreamCapacity(payload.streamCapacity ?? payload.capacity);
  }
  if (payload.streamStatus !== undefined || payload.status !== undefined) {
    updates.stream_status = normalizeStreamStatus(payload.streamStatus ?? payload.status);
  }
  if (payload.classTeacher !== undefined) {
    updates.class_teacher = normalizeClassTeacher(payload.classTeacher);
  }
  ...
  if (updates.stream_capacity !== undefined || updates.stream_status === "inactive") {
    const studentsSnap = await classRef.collection("students").get();
    const currentStudentCount = studentsSnap.docs.length;
    if (Number(current.student_count || 0) !== currentStudentCount) {
      updates.student_count = currentStudentCount;
    }
    if (updates.stream_capacity !== undefined && updates.stream_capacity < currentStudentCount) {
      throw new Error(`Stream capacity cannot be lower than the current ${currentStudentCount} students`);
    }
    if (updates.stream_status === "inactive" && currentStudentCount > 0) {
      throw new Error("Move all students before disabling this stream");
    }
  }
    ...
    if (JSON.stringify(oldSubjects) !== JSON.stringify(newSubjects)) {
      ...
      await updateDocsInBatches(db, studentsSnap.docs, (doc) => {
        ...
        const allowedSubjects = new Set(
          newSubjects.map((subject) => String(subject || "").trim().toLowerCase()),
        );
        ...
        if (Array.isArray(data.optional_subjects)) {
          updates.optional_subjects = data.optional_subjects
            .map((entry) => String(entry || "").trim())
            .filter((entry) => entry && allowedSubjects.has(entry.toLowerCase()));
        }
        return updates;
      });
    }
...
const deleteClassRecord = async (db, classId, { permanent = false } = {}) => {
  ...
  const studentsSnap = await classRef.collection("students").limit(1).get();
  if (!studentsSnap.empty) {
    throw new Error("Move all students before deleting or archiving this stream");
  }
...
const restoreClassRecord = async (db, classId) => {
  ...
  const studentsSnap = await classRef.collection("students").limit(1).get();
  if (!studentsSnap.empty) {
    throw new Error("Archived classes with students are historical records and cannot be restored");
  }
  const nextYear = current.year || DEFAULT_SCHOOL.year;
  const nextForm = current.form || DEFAULT_SCHOOL.form;
  const nextStream = validateStream(current.stream);
  await ensureUniqueClass(db, {
    year: nextYear,
    form: nextForm,
    stream: nextStream,
