...
async function buildAccessibleFormWorkspace(db, user, classRecord, examType = "") {
  if (!classRecord?.form || !classRecord?.year) {
    return classRecord;
  }

  const accessibleClasses = await getAccessibleClassSummaries(db, user);
  const relatedSummaries = accessibleClasses.filter(
    (entry) =>
      String(entry.form || "").trim() === String(classRecord.form || "").trim() &&
      String(entry.year || "").trim() === String(classRecord.year || "").trim(),
  );
  ...
  const relatedClasses = await Promise.all(
    relatedSummaries.map((entry) => getClassWithStudents(db, entry.id)),
  );

  const mergedStudents = relatedClasses.flatMap((entry) =>
    (entry.students || []).map((student) => ({
      ...student,
      id: makeMergedStudentId(entry.id, student.id),
  ...
  const subjects = Array.from(
    new Set(
      relatedClasses.flatMap((entry) => (Array.isArray(entry.subjects) ? entry.subjects : [])),
    ),
  );

  return {
    ...classRecord,
    id: `${classRecord.year}-${classRecord.form}-all-streams`,
    name: [classRecord.form, classRecord.year, "All Streams"].filter(Boolean).join(" ").trim(),
    stream: "",
    subjects,
    students: mergedStudents,
    rankTotalStudents: mergedStudents.length,
    classLabelOverride: classRecord.form || classRecord.name || "",
    relatedClasses,
    school_info: {
      ...(classRecord.school_info || classRecord.schoolInfo || {}),
...
async function getClassSummaryTool(db, user, args = {}) {
  ...
  const workspace = await buildAccessibleFormWorkspace(db, user, classRecord, examType);
  return summarizeClassPerformance(workspace, examType);
}
...
async function compareClassTrendsTool(db, user, args = {}) {
  ...
  const workspace = await buildAccessibleFormWorkspace(db, user, classRecord, currentExam);
  const current = summarizeClassPerformance(workspace, currentExam);
  const previous = summarizeClassPerformance(workspace, previousExam);
...
async function getAtRiskStudentsTool(db, user, args = {}) {
  ...
  const workspace = await buildAccessibleFormWorkspace(db, user, classRecord, examType);
  const currentStudents = buildComputedStudents(workspace, examType);
  const previousStudents = previousExam ? buildComputedStudents(workspace, previousExam) : [];
...
async function buildGuardianContactQueueTool(db, user, args = {}) {
  ...
  const limit = Math.max(1, Math.min(Number(args.limit || 30), 100));
  const workspace = await buildAccessibleFormWorkspace(db, user, classRecord, examType);
  const summary = summarizeClassPerformance(workspace, examType);
