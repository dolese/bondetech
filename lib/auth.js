...
const ADMIN_EDITABLE_FIELDS = ["displayName", "role", "email", "phone", "linkedIndexNo", "active", "teacherRoles"];
...
function normalizeTeacherRoles(value) {
  ...
  return {
    ...
    subjectTeacher: source.subjectTeacher !== false,
  };
}
...
async function bootstrapFirstAdmin(db, { username, password, rememberMe = false }, reqMeta = {}) {
  ...
  const userData = {
    ...
    phone: "",
    linkedIndexNo: "",
    teacherRoles: normalizeTeacherRoles({ subjectTeacher: true }),
    active: true,
    createdAt: now,
    updatedAt: now,
    lastLoginAt: now,
