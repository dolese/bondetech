...
// "demo" is a read-only role used by the public homepage Demo button: it can
// view class data but every write gate below excludes it.
const USER_ROLES = ["admin", "academic", "teacher", "parent", "student", "demo"];
...
let _runtimeSessionSecret = null;

function getSessionSecret() {
  if (process.env.SESSION_SECRET) return process.env.SESSION_SECRET;
  if (process.env.FIREBASE_PRIVATE_KEY) return process.env.FIREBASE_PRIVATE_KEY;
  // No configured secret: generate a per-process random secret and warn.
  // Sessions will not survive a server restart, but the secret is not guessable.
  if (!_runtimeSessionSecret) {
    _runtimeSessionSecret = require("node:crypto").randomBytes(48).toString("hex");
    console.warn(
      "[auth] WARNING: SESSION_SECRET is not set. Using a per-process random secret — " +
      "all sessions will be invalidated on restart. Set SESSION_SECRET in your environment."
    );
  }
  return _runtimeSessionSecret;
}
...
function canReadClassData(role) {
  return role === "admin" || role === "academic" || role === "teacher" || role === "demo";
}
