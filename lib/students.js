...
const normalizeAdmissionNo = (value, fallback = "") => {
...
const loadExistingStudentsByAdmissionNo = async (classRef, admissionNos) => {
  ...
  for (const chunk of chunkArray(cleaned, 30)) {
    const snapshot = await classRef
      ...
      .get();
    snapshot.docs.forEach((doc) => {
      const admissionNo = normalizeAdmissionNo(doc.data().admission_no || "");
      if (admissionNo) {
...
const generateFallbackAdmissionNo = (docId) => `${ADMISSION_PREFIX}-${String(docId || "").toUpperCase()}`;
