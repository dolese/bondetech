...
const buildMovedStudentData = ({ studentData, sourceClass, sourceClassId, studentId, targetClassSnap, cno }) => {
...
const bulkMoveStudentsToClass = async (db, assignments = [], targetClassId) => {
  ...
  if (normalizedAssignments.length > 100) {
    throw new Error("Bulk stream assignment is limited to 100 students at a time");
  }
  ...
  const cnoStart = await reserveCnoRange(db, targetClassRef, loaded.length);
...
const unassignStudentsFromStreams = async (db, assignments = []) => {
  ...
  if (normalized.length > 100) throw new Error("Bulk unassignment is limited to 100 students at a time");
...
const assignUnassignedStudentsToClass = async (db, unassignedIds = [], targetClassId) => {
  ...
  if (ids.length > 100) throw new Error("Bulk assignment is limited to 100 students at a time");
  ...
  const cnoStart = await reserveCnoRange(db, targetClassRef, unassigned.length);
  const batch = db.batch();
