...
const buildMergeUpdates = (keepData, removeDatas) => {
  ...
  // Preserve a stable CNO. A re-uploaded duplicate is usually the newest copy
  // but carries a placeholder (TMP-) or blank CNO, while the original holds the
  // stable S6509/N value. syncFormCnosForClass reassigns every CNO by sort
  // order and places non-stable CNOs last, so keeping the TMP copy would move
  // this student to the end and shift everyone else's CNO. Adopt the original's
  // stable CNO (smallest, i.e. the true original) onto the kept record.
  if (parseStableCno(keepData.index_no) == null) {
    const stableDonor = removeDatas
      .map((d) => ({ cno: sanitizeText(d.index_no || ""), n: parseStableCno(d.index_no) }))
      .filter((entry) => entry.n != null)
      .sort((a, b) => a.n - b.n)[0];
    if (stableDonor) updates.index_no = stableDonor.cno;
  }
