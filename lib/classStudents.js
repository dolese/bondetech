...
const writeStudentAuditLog = async (db, classId, classSnap, prevData, updates, payload = {}) => {
  try {
    ...
    const requestedExamType = sanitizeText(payload.examType || DEFAULT_EXAM_TYPE) || DEFAULT_EXAM_TYPE;
    const subjects = getSubjects(classSnap);
    const scoreChanges = {};
    ...
    if (nextExamScores) {
      Object.keys(nextExamScores).forEach((candidateExamType) => {
        const previousScores = Array.isArray(previousExamScores[candidateExamType]) ? previousExamScores[candidateExamType] : [];
        const nextScores = Array.isArray(nextExamScores[candidateExamType]) ? nextExamScores[candidateExamType] : [];
        ...
        for (let index = 0; index < max; index += 1) {
          ...
          scoreChanges[`score:${candidateExamType}:${subject}`] = {
            from,
            to,
            examType: candidateExamType,
            subject,
...
const updateStudentRecord = async (db, classId, studentId, payload = {}) => {
  ...
  const prevData = JSON.parse(JSON.stringify(studentSnap.data() || {}));
  const currentAdmissionNo = sanitizeText(prevData.admission_no || "");
  ...
  const auditUpdates = JSON.parse(JSON.stringify(updates));
  await studentRef.update(updates);
  await writeStudentAuditLog(db, classId, classSnap, prevData, auditUpdates, {
    ...payload,
    id: studentId,
  });
...
const bulkImportStudents = async (db, classId, rawStudents, rawExamType, rawMeta = {}) => {
  ...
  for (const raw of validStudents) {
    ...
    if (existing) {
      ...
      const existingDataForAudit = JSON.parse(JSON.stringify(existingData || {}));
      const existingExamScores =
      ...
      if (changed) {
        ...
        toUpdate.push({
          ...
          prevData: existingDataForAudit,
          updates,
          auditUpdates: JSON.parse(JSON.stringify(updates)),
          payload: auditMeta,
        });
      } else {
        skipped += 1;
  ...
  for (const entry of toUpdate) {
    await writeStudentAuditLog(db, classId, classSnap, entry.prevData, entry.auditUpdates, {
      ...entry.payload,
      id: entry.studentId,
    });
  }
