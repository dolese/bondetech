...
const moveStudentToClass = async (db, sourceClassId, studentId, targetClassId) => {
  ...
  const normalizedStudentId = sanitizeText(studentId || "");
  const normalizedTargetClassId = sanitizeText(targetClassId || "");
  if (!normalizedSourceClassId || !normalizedStudentId || !normalizedTargetClassId) {
    throw new Error("Source class, student, and target class are required");
  }
  if (normalizedSourceClassId === normalizedTargetClassId) {
    throw new Error("Source class and target class must be different");
  }

  const { classRef: sourceClassRef, classSnap: sourceClassSnap, studentRef, studentSnap } =
    await getStudentSnapshot(db, normalizedSourceClassId, normalizedStudentId);
  ...
  assertClassResultsEditable(
    sourceClassSnap,
    "Results are published for the source class. Unpublish them before moving students."
  );
  assertClassResultsEditable(
    targetClassSnap,
    "Results are published for the target class. Unpublish them before moving students."
  );

  const sourceClass = parseClass(sourceClassSnap);
  const targetClass = parseClass(targetClassSnap);
  if (
    sanitizeText(sourceClass.year || "") !== sanitizeText(targetClass.year || "") ||
    sanitizeText(sourceClass.form || "") !== sanitizeText(targetClass.form || "")
  ) {
    throw new Error("Students can only be reassigned between streams of the same form and year");
  }

  const studentData = studentSnap.data();
  const admissionNo = sanitizeAdmissionNo(studentData.admission_no || "");
  if (admissionNo) {
    const existingByAdmissionNo = await loadExistingStudentsByAdmissionNo(targetClassRef, [admissionNo]);
    if (existingByAdmissionNo[admissionNo]) {
      throw new Error("Target stream already has a student with this admission number");
    }
  }

  const targetSubjectCount = getSubjects(targetClassSnap).length;
  const existingExamScores =
    studentData.exam_scores && typeof studentData.exam_scores === "object"
      ? studentData.exam_scores
      : {};
  ...
  const sourceLabel =
    sourceClass.name || `${sourceClass.form} ${sourceClass.stream} ${sourceClass.year}`.trim();
  const targetLabel =
    targetClass.name || `${targetClass.form} ${targetClass.stream} ${targetClass.year}`.trim();
  const movedAt = new Date().toISOString();
  const nextCnoStart = await reserveCnoRange(db, targetClassRef, 1);
  const targetStudentRef = targetClassRef.collection("students").doc();
  const movedStudent = {
    ...studentData,
    index_no: formatCno(nextCnoStart),
    admission_no: admissionNo,
    optional_subjects: optionalSubjects,
    exam_scores: examScores,
    scores: sanitizeScores(
      examScores[DEFAULT_EXAM_TYPE] || studentData.scores || [],
      targetSubjectCount
    ),
    updated_at: movedAt,
    stream_transfer_from: {
      classId: normalizedSourceClassId,
      classLabel: sourceLabel,
  ...
  batch.update(sourceClassRef, {
    student_count: Math.max(0, Number(sourceClassSnap.data().student_count || 0) - 1),
  });
  batch.update(targetClassRef, {
    student_count: Number(targetClassSnap.data().student_count || 0) + 1,
  });
  await batch.commit();
  ...
  return {
    ...
    sourceClassId: normalizedSourceClassId,
    targetClassId: normalizedTargetClassId,
    studentName: sanitizeText(studentData.name || ""),
    movedAt,
  };
};
...
const promoteStudentsToClass = async (db, sourceClassId, targetClassId) => {
  ...
  let nextCno = 0;
  if (toCreate.length) {
    nextCno = await reserveCnoRange(db, targetClassRef, toCreate.length);
  }
  toCreate.forEach((student, index) => {
    student.index_no = formatCno(nextCno + index);
  });
