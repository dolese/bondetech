...
const buildMergeUpdates = (keepData, removeDatas) => {
  ...
  // Per-exam scores: keep the newest record's marks for any exam it already
  // has; pull in exams that only exist on the removed copies.
  const keepExamScores =
    keepData.exam_scores && typeof keepData.exam_scores === "object"
      ? { ...keepData.exam_scores }
      : {};
  let examScoresChanged = false;
  const examTypes = new Set();
  ordered.forEach((d) => {
    if (d.exam_scores && typeof d.exam_scores === "object") {
      Object.keys(d.exam_scores).forEach((exam) => examTypes.add(exam));
    }
  });
  for (const exam of examTypes) {
    if (countFilledScores(keepExamScores[exam]) > 0) continue;
    const donor = ordered.find((d) => countFilledScores(d.exam_scores?.[exam]) > 0);
    if (donor) {
      keepExamScores[exam] = donor.exam_scores[exam];
