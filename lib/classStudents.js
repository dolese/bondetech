...
const bulkImportStudentMarks = async (db, classId, rawStudents, rawExamType) => {
  const { classRef, classSnap } = await getClassSnapshot(db, classId);
  assertClassResultsEditable(
    classSnap,
    "Results are published for this class. Unpublish them before importing marks.",
  );
  ...
  const requestedIndexNos = students
    .map((student) => sanitizeText(student.indexNo || student.index_no || ""))
    .filter(Boolean);
  const requestedAdmissionNos = students
  ...
  for (const raw of students) {
    ...
    const incomingAdmissionNo = sanitizeAdmissionNo(raw.admissionNo ?? raw.admission_no ?? "");
    if (incomingAdmissionNo && !isValidAdmissionNo(incomingAdmissionNo)) {
      throw new Error("Admission number must use the format SCHOOLCODE-YEAR-SEQUENCE, for example BSS-2026-0001");
    }
    if (!incomingAdmissionNo && !incomingIndexNo) {
      invalid += 1;
      continue;
    }

    const existingByAdmission = incomingAdmissionNo ? existingByAdmissionNo[incomingAdmissionNo] : null;
    ...
    if (
      ...
      throw new Error(
        `Imported row points to different students by CNO and Admission Number`
      );
    }
    ...
    if (JSON.stringify(newScores) === JSON.stringify(existingExamScoreForType)) {
      skipped += 1;
      continue;
    }
...
const bulkImportStudents = async (db, classId, rawStudents, rawExamType, options = {}) => {
  if (options?.mode === "marks-only") {
    return bulkImportStudentMarks(db, classId, rawStudents, rawExamType);
  }
...
const buildMergeUpdates = (keepData, removeDatas) => {
  ...
  // Preserve sex. It has no "empty" state — bulkImportStudents defaults an
  // omitted sex column to "M", so a re-uploaded kept copy may be a defaulted
  // "M" while the original was explicitly "F" (F is never a default). If the
  // kept value is "M" and a removed copy carries "F", adopt "F" so a female
  // original isn't silently made male (which would also skew sex-grouped CNO
  // ordering). Only the M->F direction is overridden, since M is the ambiguous
  // default and F is always explicit.
  if (keepData.sex !== "F" && removeDatas.some((d) => d.sex === "F")) {
    updates.sex = "F";
  }

  // Preserve non-default statuses. bulkImportStudents defaults an omitted
  // enrollment_status to "active" and status to "present", so a re-uploaded
  // kept copy may carry those defaults while the original recorded a real
  // transferred/left/graduated enrollment or an absent/incomplete status.
  // Adopt a removed copy's non-default value when the kept copy holds the
  // default (only that direction, since the default is the ambiguous value).
  if (keepData.enrollment_status === "active" || isEmptyValue(keepData.enrollment_status)) {
    const donor = ordered.find(
      (d) => VALID_ENROLLMENT_STATUSES.includes(d.enrollment_status) && d.enrollment_status !== "active"
    );
    if (donor) updates.enrollment_status = donor.enrollment_status;
  }
  if (keepData.status === "present" || isEmptyValue(keepData.status)) {
    const donor = ordered.find(
      (d) => VALID_STATUSES.includes(d.status) && d.status !== "present"
    );
    if (donor) updates.status = donor.status;
  }

  // Preserve the earliest stable CNO. A re-uploaded duplicate may carry a
  // placeholder (TMP-) CNO, but because bulkImportStudents runs
  // syncFormCnosForClass right after import, the newer copy often already has a
  // stable end-of-list S6509/N while the original keeps its smaller value. The
  // next CNO sync orders by that number, so the kept record must hold the
  // SMALLEST stable CNO across all copies to keep the student's slot and avoid
  // shifting intervening students. Compare kept + removed and adopt the
  // earliest, whether or not the kept copy already has a stable CNO.
  {
    const earliest = [keepData, ...removeDatas]
      .map((d) => ({ cno: sanitizeText(d.index_no || ""), n: parseStableCno(d.index_no) }))
      .filter((entry) => entry.n != null)
      .sort((a, b) => a.n - b.n)[0];
    if (earliest && earliest.cno !== sanitizeText(keepData.index_no || "")) {
      updates.index_no = earliest.cno;
    }
  }

  // Conduct is an object of per-trait grades. A default record carries all the
  // keys with blank values, so an object-has-keys check would wrongly treat it
  // as populated. Merge per trait: keep the kept record's grade where set,
  // backfill each blank trait from a removed copy that has a real grade.
  {
    const keepConduct =
      keepData.conduct && typeof keepData.conduct === "object" ? { ...keepData.conduct } : {};
    const conductKeys = new Set(Object.keys(keepConduct));
    ordered.forEach((d) => {
      if (d.conduct && typeof d.conduct === "object") {
        Object.keys(d.conduct).forEach((key) => conductKeys.add(key));
      }
    });
    let conductChanged = false;
    for (const key of conductKeys) {
      if (!isEmptyScore(keepConduct[key])) continue;
      const donor = ordered.find((d) => d.conduct && !isEmptyScore(d.conduct[key]));
      if (donor) {
        keepConduct[key] = donor.conduct[key];
        conductChanged = true;
      }
    }
    if (conductChanged) updates.conduct = keepConduct;
  }
  // Optional subjects decide which score slots are active, so union them across
  // the kept and removed copies rather than treating the field as
  // all-or-nothing. The kept record's own subjects come first; each removed
  // copy contributes any extras (case-insensitive de-dupe).
  {
    const seen = new Set();
    const union = [];
    let addedFromDonor = false;
    const collect = (list, fromKeep) => {
      (Array.isArray(list) ? list : []).forEach((entry) => {
        ...
        seen.add(key);
        union.push(value);
        if (!fromKeep) addedFromDonor = true;
      });
    };
    collect(keepData.optional_subjects, true);
    ordered.forEach((d) => collect(d.optional_subjects, false));
    if (addedFromDonor) updates.optional_subjects = union;
  }
  ...
  for (const exam of examTypes) {
    ...
    const keepArr = Array.isArray(keepExamScores[exam]) ? keepExamScores[exam] : [];
    const maxLen = Math.max(keepArr.length, ...donorArrays.map((arr) => arr.length));
    // Build a DENSE array: every slot is assigned so there are no undefined
    // holes (Firestore rejects undefined array entries on update). The kept
    // record's own mark wins; empty slots are backfilled from donors, and any
    // slot no one filled becomes a blank string.
    const merged = [];
    let slotChanged = false;
    for (let i = 0; i < maxLen; i += 1) {
      const keepVal = keepArr[i];
      if (!isEmptyScore(keepVal)) {
        merged[i] = keepVal;
        continue;
      }
    ...
    // Adopt the merged array when a slot changed, or when the kept record had no
    // array for this exam at all — but only if it actually carries a mark.
