...
const findDuplicateStudentGroups = async (db, classId) => {
  ...
  for (const members of byName.values()) {
    ...
    groups.push({ name: keep.name, keep, remove });
  }
...
const bulkDeleteStudents = async (db, classId, studentIds = []) => {
  const { classRef, classSnap } = await getClassSnapshot(db, classId);
  assertClassResultsEditable(
    classSnap,
    "Results are published for this class. Unpublish them before removing students."
  );
  // syncFormCnosForClass (run after the deletes) re-checks every stream in this
  // form/year and throws if any sibling is published. Preflight that check here
  // so a published sibling fails the request before anything is deleted, keeping
  ...
  if (!ids.length) {
    throw new Error("At least one student is required to remove duplicates");
  }

  // Verify every id exists in this class before deleting anything.
  const refs = ids.map((id) => classRef.collection("students").doc(id));
  ...
  await classRef.update({
    student_count: Math.max(0, Number(classSnap.data().student_count || 0) - refs.length),
  });
  await syncFormCnosForClass(db, classId);
