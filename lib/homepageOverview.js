...
const HERO_ACTIONS = new Set(["results", "login", "announcements", "contact"]);
const DEFAULT_SLIDE_IMAGES = ["/asset/nembobonde.jpg", "/asset/slider2.png", "/asset/slider3.png"];
const LEGACY_PRIMARY_SLIDE_IMAGES = new Set([
  "https://github.com/user-attachments/assets/78f7a406-6268-4b6a-a281-70ba41089e6f",
  "/asset/slider1.png",
]);
...
function normalizeSlideAction(value, fallback = "results") {
  const action = String(value || "").trim().toLowerCase();
  return HERO_ACTIONS.has(action) ? action : fallback;
}

function normalizeSlideImage(value, index) {
  const fallback = DEFAULT_SLIDE_IMAGES[index] || DEFAULT_SLIDE_IMAGES[0];
  const imageSrc = normalizeText(value, fallback, 255);
  if (index === 0 && LEGACY_PRIMARY_SLIDE_IMAGES.has(imageSrc)) {
    return DEFAULT_SLIDE_IMAGES[0];
  }
  return imageSrc || fallback;
}

function sanitizeSlides(items, fallbackItems = []) {
  const source = Array.isArray(items) && items.length > 0 ? items : fallbackItems;
  return source
    .map((item, index) => {
      const imageSrc = normalizeSlideImage(item?.imageSrc, index);
      if (!imageSrc) return null;
      return {
        id: normalizeText(item?.id, createStableId("slide", index), 60),
        imageSrc,
        badge: normalizeText(item?.badge, "", 40),
        badgeSw: normalizeText(item?.badgeSw, "", 40),
        title: normalizeText(item?.title, "", 120),
        titleSw: normalizeText(item?.titleSw, "", 120),
        description: normalizeText(item?.description, "", 220),
        descriptionSw: normalizeText(item?.descriptionSw, "", 220),
        primaryAction: normalizeSlideAction(item?.primaryAction, "results"),
        secondaryAction: normalizeSlideAction(item?.secondaryAction, "login"),
        backgroundPosition: normalizeText(item?.backgroundPosition, "center", 40),
      };
    })
    .filter(Boolean)
    .slice(0, 3);
...
function sanitizeImages(items, fallbackItems = []) {
  const source = Array.isArray(items) && items.length > 0 ? items : fallbackItems;
  return source
    .map((item, index) => {
      const src = normalizeText(item?.src, "", 500);
      if (!src) return null;
      return {
...
function buildHomepageContent(input, defaults = {}) {
  return {
    ...
    images: sanitizeImages(input?.images, defaults.images || []),
    slides: sanitizeSlides(
      input?.slides,
      defaults.slides ||
        DEFAULT_SLIDE_IMAGES.map((imageSrc, index) => ({
          id: createStableId("slide", index),
          imageSrc,
        }))
    ),
